...
Reserved `main` delivery on Cursor or Grok preempts an in-flight TUI turn before `send`, `assign`, `message send`, or inbound drain. Cursor uses tmux `C-c`. Grok uses `Escape` (cancel immediately; a bare Enter while busy only queues a follow-up) and, if the pane is still busy, an empty native Enter to send-now the just-queued row. Messages that start with `# FractalBot Heartbeat` are not preempted. Employee agents are unchanged.
