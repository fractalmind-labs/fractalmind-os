...
def find_skill_dir(skill_name: str, repo_root: Optional[Path]) -> Optional[Path]:
...
def find_agent_manager_scripts_dir(repo_root: Optional[Path]) -> Optional[Path]:
    skill_dir = find_skill_dir("agent-manager", repo_root)
    if not skill_dir:
        return None
