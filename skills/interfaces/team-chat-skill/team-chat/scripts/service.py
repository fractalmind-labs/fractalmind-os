...
This module intentionally avoids dependencies on the rest of the codebase so that
scripts can import it without creating import cycles.
"""
...
def update_service_state(state_dir: Path, ok: bool, error: Optional[str] = None) -> ServiceStateResult:
    ...
    Files written (atomic):
    - unread_notifier.last_run  (epoch seconds, always)
    - unread_notifier.last_ok   (epoch seconds, only when ok)
    - unread_notifier.fail_count (consecutive failures; reset to 0 on success)

    Return value is suitable for `--json` output.
    """
