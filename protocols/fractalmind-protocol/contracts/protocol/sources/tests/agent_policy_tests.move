...
module fractalmind_protocol::agent_policy_tests {
    ...
    fun test_create_policy_and_execute_action() {
        ...
        ts::next_tx(&mut scenario, AGENT1);
        {
            ...
            assert!(agent_policy::policy_allowed_action(&policy_obj) == string::utf8(b"shell_exec"), 2);
            assert!(agent_policy::policy_target_scope(&policy_obj) == string::utf8(b"host:worker-1"), 3);
            assert!(agent_policy::policy_uses_consumed(&policy_obj) == 1, 4);
            assert!(!agent_policy::policy_revoked(&policy_obj), 5);
    ...
    fun test_revoke_policy() {
        ...
        ts::next_tx(&mut scenario, ADMIN);
        {
            ...
            assert!(agent_policy::policy_revoked(&policy_obj), 0);
            ts::return_shared(org);
            ts::return_shared(policy_obj);
        };
