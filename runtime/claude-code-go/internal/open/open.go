...
type Result struct {
	...
	HookBlockingErrorValidated                                   bool
	HookBlockingErrorEvent                                       string
	HookErrorDuringExecutionValidated                            bool
	HookErrorDuringExecutionEvent                                string
	HookNonBlockingErrorValidated                                bool
	HookNonBlockingErrorEvent                                    string
	HookCancelledValidated                                       bool
	HookCancelledEvent                                           string
	HookSuccessValidated                                         bool
	HookSuccessEvent                                             string
	HookPermissionDecisionValidated                              bool
	HookPermissionDecisionEvent                                  string
	HookStoppedContinuationValidated                             bool
	HookStoppedContinuationEvent                                 string
	HookSystemMessageValidated                                   bool
	HookSystemMessageEvent                                       string
...
func Run(args []string) (Result, error) {
	...
	return Result{
		...
		HookBlockingErrorValidated:                             streamResult.HookBlockingErrorValidated,
		HookBlockingErrorEvent:                                 streamResult.HookBlockingErrorEvent,
		HookErrorDuringExecutionValidated:                      streamResult.HookErrorDuringExecutionValidated,
		HookErrorDuringExecutionEvent:                          streamResult.HookErrorDuringExecutionEvent,
		HookNonBlockingErrorValidated:                          streamResult.HookNonBlockingErrorValidated,
		HookNonBlockingErrorEvent:                              streamResult.HookNonBlockingErrorEvent,
		HookCancelledValidated:                                 streamResult.HookCancelledValidated,
		HookCancelledEvent:                                     streamResult.HookCancelledEvent,
		HookSuccessValidated:                                   streamResult.HookSuccessValidated,
		HookSuccessEvent:                                       streamResult.HookSuccessEvent,
		HookPermissionDecisionValidated:                        streamResult.HookPermissionDecisionValidated,
		HookPermissionDecisionEvent:                            streamResult.HookPermissionDecisionEvent,
		HookStoppedContinuationValidated:                       streamResult.HookStoppedContinuationValidated,
		HookStoppedContinuationEvent:                           streamResult.HookStoppedContinuationEvent,
		HookSystemMessageValidated:                             streamResult.HookSystemMessageValidated,
		HookSystemMessageEvent:                                 streamResult.HookSystemMessageEvent,
...
func validateStream(rawWSURL, authToken string, opts Options) (streamValidation, error) {
	...
	for _, turn := range turns {
		...
		hookBlockingErrorValidated := false
		hookErrorDuringExecutionValidated := false
		hookNonBlockingErrorValidated := false
		hookCancelledValidated := false
		hookSuccessValidated := false
		hookStoppedContinuationValidated := false
		hookSystemMessageValidated := false
		hookAdditionalContextValidated := false
		hookPermissionDecisionValidated := false
		...
		for {
			...
			case "user":
				...
				if !replayed {
					...
					if len(content) > 0 {
						...
						if strings.TrimSpace(asString(firstBlock["type"])) == "tool_result" {
							if strings.TrimSpace(asString(incoming["timestamp"])) == "" {
								return streamValidation{}, fmt.Errorf("invalid live tool_result: missing timestamp")
							}
							if synthetic {
								return streamValidation{}, fmt.Errorf("invalid live tool_result: expected isSynthetic=false")
							}
							if err := validateParentToolUseIDNull(incoming, "invalid live tool_result"); err != nil {
								return streamValidation{}, err
							}
			...
			case "attachment":
				...
				attachmentType := strings.TrimSpace(asString(attachment["type"]))
				if turn.behavior == "deny" && attachmentType != "hook_permission_decision" {
				...
				case "output_style":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected output_style attachment during %s turn", turn.behavior)
					}
					if strings.TrimSpace(asString(attachment["style"])) != "explanatory" {
						return streamValidation{}, fmt.Errorf("invalid output_style attachment.style")
					}
				...
				case "hook_permission_decision":
					if turn.behavior != "allow" && turn.behavior != "deny" {
						return streamValidation{}, fmt.Errorf("unexpected hook_permission_decision attachment during %s turn", turn.behavior)
					}
					expectedDecision := turn.behavior
					if strings.TrimSpace(asString(attachment["decision"])) != expectedDecision {
						return streamValidation{}, fmt.Errorf("invalid hook_permission_decision attachment.decision")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != currentToolUseID {
						return streamValidation{}, fmt.Errorf("invalid hook_permission_decision attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["hookEvent"])) != "PermissionRequest" {
						return streamValidation{}, fmt.Errorf("invalid hook_permission_decision attachment.hookEvent")
					}
					result.HookPermissionDecisionValidated = true
					result.HookPermissionDecisionEvent = "attachment:hook_permission_decision"
					hookPermissionDecisionValidated = true
				case "hook_blocking_error":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected hook_blocking_error attachment during %s turn", turn.behavior)
					}
					blockingError, _ := attachment["blockingError"].(map[string]any)
					if strings.TrimSpace(asString(attachment["hookName"])) != "DirectConnectEchoHook" {
						return streamValidation{}, fmt.Errorf("invalid hook_blocking_error attachment.hookName")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != currentToolUseID {
						return streamValidation{}, fmt.Errorf("invalid hook_blocking_error attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["hookEvent"])) != "Stop" {
						return streamValidation{}, fmt.Errorf("invalid hook_blocking_error attachment.hookEvent")
					}
					if strings.TrimSpace(asString(blockingError["blockingError"])) != "Direct-connect demo hook blocked execution." {
						return streamValidation{}, fmt.Errorf("invalid hook_blocking_error attachment.blockingError.blockingError")
					}
					if strings.TrimSpace(asString(blockingError["command"])) != "direct-connect-demo-hook" {
						return streamValidation{}, fmt.Errorf("invalid hook_blocking_error attachment.blockingError.command")
					}
					result.HookBlockingErrorValidated = true
					result.HookBlockingErrorEvent = "attachment:hook_blocking_error"
					hookBlockingErrorValidated = true
				case "hook_error_during_execution":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected hook_error_during_execution attachment during %s turn", turn.behavior)
					}
					if strings.TrimSpace(asString(attachment["content"])) != "Direct-connect demo hook failed before completing execution." {
						return streamValidation{}, fmt.Errorf("invalid hook_error_during_execution attachment.content")
					}
					if strings.TrimSpace(asString(attachment["hookName"])) != "DirectConnectEchoHook" {
						return streamValidation{}, fmt.Errorf("invalid hook_error_during_execution attachment.hookName")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != currentToolUseID {
						return streamValidation{}, fmt.Errorf("invalid hook_error_during_execution attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["hookEvent"])) != "Stop" {
						return streamValidation{}, fmt.Errorf("invalid hook_error_during_execution attachment.hookEvent")
					}
					result.HookErrorDuringExecutionValidated = true
					result.HookErrorDuringExecutionEvent = "attachment:hook_error_during_execution"
					hookErrorDuringExecutionValidated = true
				case "hook_non_blocking_error":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected hook_non_blocking_error attachment during %s turn", turn.behavior)
					}
					if strings.TrimSpace(asString(attachment["hookName"])) != "DirectConnectEchoHook" {
						return streamValidation{}, fmt.Errorf("invalid hook_non_blocking_error attachment.hookName")
					}
					if strings.TrimSpace(asString(attachment["stderr"])) != "Direct-connect demo hook reported a recoverable issue." {
						return streamValidation{}, fmt.Errorf("invalid hook_non_blocking_error attachment.stderr")
					}
					if strings.TrimSpace(asString(attachment["stdout"])) != turn.expectedResponse {
						return streamValidation{}, fmt.Errorf("invalid hook_non_blocking_error attachment.stdout")
					}
					if fmt.Sprint(attachment["exitCode"]) != "1" {
						return streamValidation{}, fmt.Errorf("invalid hook_non_blocking_error attachment.exitCode")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != currentToolUseID {
						return streamValidation{}, fmt.Errorf("invalid hook_non_blocking_error attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["hookEvent"])) != "Stop" {
						return streamValidation{}, fmt.Errorf("invalid hook_non_blocking_error attachment.hookEvent")
					}
					result.HookNonBlockingErrorValidated = true
					result.HookNonBlockingErrorEvent = "attachment:hook_non_blocking_error"
					hookNonBlockingErrorValidated = true
				case "hook_cancelled":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected hook_cancelled attachment during %s turn", turn.behavior)
					}
					if strings.TrimSpace(asString(attachment["hookName"])) != "DirectConnectEchoHook" {
						return streamValidation{}, fmt.Errorf("invalid hook_cancelled attachment.hookName")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != currentToolUseID {
						return streamValidation{}, fmt.Errorf("invalid hook_cancelled attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["hookEvent"])) != "Stop" {
						return streamValidation{}, fmt.Errorf("invalid hook_cancelled attachment.hookEvent")
					}
					result.HookCancelledValidated = true
					result.HookCancelledEvent = "attachment:hook_cancelled"
					hookCancelledValidated = true
				case "hook_success":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected hook_success attachment during %s turn", turn.behavior)
					}
					if strings.TrimSpace(asString(attachment["content"])) != turn.expectedResponse {
						return streamValidation{}, fmt.Errorf("invalid hook_success attachment.content")
					}
					if strings.TrimSpace(asString(attachment["hookName"])) != "DirectConnectEchoHook" {
						return streamValidation{}, fmt.Errorf("invalid hook_success attachment.hookName")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != currentToolUseID {
						return streamValidation{}, fmt.Errorf("invalid hook_success attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["hookEvent"])) != "Stop" {
						return streamValidation{}, fmt.Errorf("invalid hook_success attachment.hookEvent")
					}
					result.HookSuccessValidated = true
					result.HookSuccessEvent = "attachment:hook_success"
					hookSuccessValidated = true
				case "hook_stopped_continuation":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected hook_stopped_continuation attachment during %s turn", turn.behavior)
					}
					if strings.TrimSpace(asString(attachment["message"])) != "Execution stopped by DirectConnectEchoHook." {
						return streamValidation{}, fmt.Errorf("invalid hook_stopped_continuation attachment.message")
					}
					if strings.TrimSpace(asString(attachment["hookName"])) != "DirectConnectEchoHook" {
						return streamValidation{}, fmt.Errorf("invalid hook_stopped_continuation attachment.hookName")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != currentToolUseID {
						return streamValidation{}, fmt.Errorf("invalid hook_stopped_continuation attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["hookEvent"])) != "Stop" {
						return streamValidation{}, fmt.Errorf("invalid hook_stopped_continuation attachment.hookEvent")
					}
					result.HookStoppedContinuationValidated = true
					result.HookStoppedContinuationEvent = "attachment:hook_stopped_continuation"
					hookStoppedContinuationValidated = true
				case "hook_system_message":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected hook_system_message attachment during %s turn", turn.behavior)
					}
					if strings.TrimSpace(asString(attachment["content"])) != "Direct-connect echo stop hook acknowledged." {
						return streamValidation{}, fmt.Errorf("invalid hook_system_message attachment.content")
					}
					if strings.TrimSpace(asString(attachment["hookName"])) != "DirectConnectEchoHook" {
						return streamValidation{}, fmt.Errorf("invalid hook_system_message attachment.hookName")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != currentToolUseID {
						return streamValidation{}, fmt.Errorf("invalid hook_system_message attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["hookEvent"])) != "Stop" {
						return streamValidation{}, fmt.Errorf("invalid hook_system_message attachment.hookEvent")
					}
					result.HookSystemMessageValidated = true
					result.HookSystemMessageEvent = "attachment:hook_system_message"
					hookSystemMessageValidated = true
				case "async_hook_response":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected async_hook_response attachment during %s turn", turn.behavior)
					}
					if strings.TrimSpace(asString(attachment["hookName"])) != "PostToolUse" {
						return streamValidation{}, fmt.Errorf("invalid async_hook_response attachment.hookName")
					}
					if strings.TrimSpace(asString(attachment["sessionId"])) != strings.TrimSpace(asString(incoming["session_id"])) {
						return streamValidation{}, fmt.Errorf("invalid async_hook_response attachment.sessionId")
					}
					if strings.TrimSpace(asString(attachment["toolUseID"])) != "toolu_demo_async_hook" {
						return streamValidation{}, fmt.Errorf("invalid async_hook_response attachment.toolUseID")
					}
					if strings.TrimSpace(asString(attachment["content"])) != "Async hook completed: captured post-tool summary." {
						return streamValidation{}, fmt.Errorf("invalid async_hook_response attachment.content")
					}
				...
				case "current_session_memory":
					...
					if strings.TrimSpace(asString(attachment["content"])) != "Remember: keep this session focused." {
						return streamValidation{}, fmt.Errorf("invalid current_session_memory attachment.content")
					}
					if strings.TrimSpace(asString(attachment["path"])) != "MEMORY.md" {
						return streamValidation{}, fmt.Errorf("invalid current_session_memory attachment.path")
					}
					tokenCount, ok := attachment["tokenCount"].(float64)
					if !ok || int(tokenCount) != 7 {
						return streamValidation{}, fmt.Errorf("invalid current_session_memory attachment.tokenCount")
					}
				...
				case "team_context":
					...
					if strings.TrimSpace(asString(attachment["agentId"])) != "agent-dev" {
						return streamValidation{}, fmt.Errorf("invalid team_context attachment.agentId")
					}
					if strings.TrimSpace(asString(attachment["agentName"])) != "dev" {
						return streamValidation{}, fmt.Errorf("invalid team_context attachment.agentName")
					}
					if strings.TrimSpace(asString(attachment["teamName"])) != "alpha" {
						return streamValidation{}, fmt.Errorf("invalid team_context attachment.teamName")
					}
					if strings.TrimSpace(asString(attachment["teamConfigPath"])) != ".claude/team.yaml" {
						return streamValidation{}, fmt.Errorf("invalid team_context attachment.teamConfigPath")
					}
					if strings.TrimSpace(asString(attachment["taskListPath"])) != ".claude/tasks.json" {
						return streamValidation{}, fmt.Errorf("invalid team_context attachment.taskListPath")
					}
					result.TeamContextValidated = true
					result.TeamContextEvent = "attachment:team_context"
					teamContextValidated = true
				case "skill_discovery":
					if turn.behavior != "allow" {
						return streamValidation{}, fmt.Errorf("unexpected skill_discovery attachment during %s turn", turn.behavior)
					}
					skills, ok := attachment["skills"].([]any)
					if !ok || len(skills) != 1 {
						return streamValidation{}, fmt.Errorf("invalid skill_discovery attachment.skills")
					}
					skill, ok := skills[0].(map[string]any)
					if !ok {
						return streamValidation{}, fmt.Errorf("invalid skill_discovery attachment.skills[0]")
					}
					if strings.TrimSpace(asString(skill["name"])) != "agent-manager" {
						return streamValidation{}, fmt.Errorf("invalid skill_discovery attachment.skills[0].name")
					}
					if strings.TrimSpace(asString(skill["description"])) != "Coordinate and track teammate work." {
						return streamValidation{}, fmt.Errorf("invalid skill_discovery attachment.skills[0].description")
					}
					if strings.TrimSpace(asString(skill["shortId"])) != "am" {
						return streamValidation{}, fmt.Errorf("invalid skill_discovery attachment.skills[0].shortId")
					}
					if strings.TrimSpace(asString(attachment["signal"])) != "user_input" {
						return streamValidation{}, fmt.Errorf("invalid skill_discovery attachment.signal")
					}
					if strings.TrimSpace(asString(attachment["source"])) != "native" {
						return streamValidation{}, fmt.Errorf("invalid skill_discovery attachment.source")
					}
...
func (r Result) String() string {
	...
	b.WriteString(fmt.Sprintf("hook_blocking_error_validated=%t\n", r.HookBlockingErrorValidated))
	b.WriteString(fmt.Sprintf("hook_blocking_error_event=%s\n", valueOrNone(r.HookBlockingErrorEvent)))
	b.WriteString(fmt.Sprintf("hook_error_during_execution_validated=%t\n", r.HookErrorDuringExecutionValidated))
	b.WriteString(fmt.Sprintf("hook_error_during_execution_event=%s\n", valueOrNone(r.HookErrorDuringExecutionEvent)))
	b.WriteString(fmt.Sprintf("hook_non_blocking_error_validated=%t\n", r.HookNonBlockingErrorValidated))
	b.WriteString(fmt.Sprintf("hook_non_blocking_error_event=%s\n", valueOrNone(r.HookNonBlockingErrorEvent)))
	b.WriteString(fmt.Sprintf("hook_cancelled_validated=%t\n", r.HookCancelledValidated))
	b.WriteString(fmt.Sprintf("hook_cancelled_event=%s\n", valueOrNone(r.HookCancelledEvent)))
	b.WriteString(fmt.Sprintf("hook_success_validated=%t\n", r.HookSuccessValidated))
	b.WriteString(fmt.Sprintf("hook_success_event=%s\n", valueOrNone(r.HookSuccessEvent)))
	b.WriteString(fmt.Sprintf("hook_permission_decision_validated=%t\n", r.HookPermissionDecisionValidated))
	b.WriteString(fmt.Sprintf("hook_permission_decision_event=%s\n", valueOrNone(r.HookPermissionDecisionEvent)))
	b.WriteString(fmt.Sprintf("hook_stopped_continuation_validated=%t\n", r.HookStoppedContinuationValidated))
	b.WriteString(fmt.Sprintf("hook_stopped_continuation_event=%s\n", valueOrNone(r.HookStoppedContinuationEvent)))
	b.WriteString(fmt.Sprintf("hook_system_message_validated=%t\n", r.HookSystemMessageValidated))
	b.WriteString(fmt.Sprintf("hook_system_message_event=%s\n", valueOrNone(r.HookSystemMessageEvent)))
