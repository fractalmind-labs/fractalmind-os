...
func TestRunOpenDefaults(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"invoked_skills_validated=false",
		...
		"hook_blocking_error_validated=false",
		"hook_error_during_execution_validated=false",
		"hook_non_blocking_error_validated=false",
		"hook_cancelled_validated=false",
		"hook_success_validated=false",
		"hook_permission_decision_validated=false",
		"hook_stopped_continuation_validated=false",
		"hook_system_message_validated=false",
		...
		"live_tool_result_validated=false",
		"live_tool_result_error_validated=false",
		...
		case "control_response":
			...
			if strings.TrimSpace(asString(responsePayload["behavior"])) == "deny" {
				_ = conn.WriteJSON(map[string]any{
					...
					"uuid":       fmt.Sprintf("hook-permission-decision-deny-%d", requestCounter),
					"session_id": sessionID,
				})
				_ = conn.WriteJSON(map[string]any{
					"type":               "user",
					"isReplay":           false,
					"isSynthetic":        false,
					"uuid":               fmt.Sprintf("deny-live-tool-result-%d", requestCounter),
					"session_id":         sessionID,
					"parent_tool_use_id": nil,
					"timestamp":          time.Now().UTC().Format(time.RFC3339Nano),
					"message": map[string]any{
						...
						"content": []map[string]any{
							{
								"type":        "tool_result",
								"tool_use_id": fmt.Sprintf("toolu-%d", requestCounter),
								"content":     "permission denied for tool echo",
								"is_error":    true,
							},
						},
					},
			...
			_ = conn.WriteJSON(map[string]any{
				...
				"uuid":       fmt.Sprintf("hook-permission-decision-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
			...
			_ = conn.WriteJSON(map[string]any{
				...
				"isReplay":           false,
				"isSynthetic":        false,
				"uuid":               fmt.Sprintf("live-tool-result-%d", requestCounter),
				"session_id":         sessionID,
				"parent_tool_use_id": nil,
				"timestamp":          time.Now().UTC().Format(time.RFC3339Nano),
				"message": map[string]any{
			...
			_ = conn.WriteJSON(map[string]any{
				...
				"uuid":       fmt.Sprintf("agent-mention-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type":    "system",
			...
			_ = conn.WriteJSON(map[string]any{
				...
				"uuid":       fmt.Sprintf("invoked-skills-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type": "attachment",
			...
			_ = conn.WriteJSON(map[string]any{
				...
				"uuid":       fmt.Sprintf("hook-blocking-error-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type": "attachment",
				"attachment": map[string]any{
					...
					"content":   "Direct-connect demo hook failed before completing execution.",
					"hookName":  "DirectConnectEchoHook",
					"toolUseID": fmt.Sprintf("toolu-%d", requestCounter),
					"hookEvent": "Stop",
				},
				"uuid":       fmt.Sprintf("hook-error-during-execution-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type": "attachment",
				...
				"uuid":       fmt.Sprintf("hook-non-blocking-error-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type": "attachment",
				...
				"uuid":       fmt.Sprintf("hook-cancelled-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type": "attachment",
				...
				"uuid":       fmt.Sprintf("hook-success-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type": "attachment",
				"attachment": map[string]any{
					...
					"message":   "Execution stopped by DirectConnectEchoHook.",
					"hookName":  "DirectConnectEchoHook",
					"toolUseID": fmt.Sprintf("toolu-%d", requestCounter),
					"hookEvent": "Stop",
				},
				"uuid":       fmt.Sprintf("hook-stopped-continuation-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type": "attachment",
				...
				"uuid":       fmt.Sprintf("hook-system-message-%d", requestCounter),
				"session_id": sessionID,
			})
			_ = conn.WriteJSON(map[string]any{
				"type": "attachment",
