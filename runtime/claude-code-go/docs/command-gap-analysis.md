...
- 本轮继续补了最小 `attachment:hook_error_during_execution`：server 在 `system:hook_response` 后、`attachment:hook_non_blocking_error` 前发出 `attachment{type:"hook_error_during_execution",content,hookName,toolUseID,hookEvent}`；`open --print` 新增 `hook_error_during_execution_validated=true / hook_error_during_execution_event=attachment:hook_error_during_execution`，并要求该 attachment 与既有 `system:hook_started / system:hook_progress / system:hook_response` 同时命中。
