...
### 下一步要补的高频子路径：direct-connect 的 `compact_boundary / hook_response` 等更丰富系统事件

选择理由：
1. `server ↔ open` 现在已经形成最小 `/sessions + websocket ready/control/message + lockfile + session index + reconnect + detached-state + backend process lifecycle + system:init + auth_status + system:status + keep_alive + control_cancel_request + tool_progress + rate_limit_event + stream_event + tool_use_summary + result(success/error_during_execution) + system(post_turn_summary)` 闭环，并且 `max_sessions` 容量保护与单 session stop/cleanup 都已补齐
2. post_turn_summary 收口后，最直接的下一个真实缺口变成“系统事件 still too thin”：当前离完整 direct-connect 体验还缺 `compact_boundary / hook_response` 等更丰富状态机输出
