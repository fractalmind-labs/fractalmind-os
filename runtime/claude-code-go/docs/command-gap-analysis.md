...
当前建议的下一个最小切片：`attachment:task_status`
- 理由：官方 `utils/attachments.ts` 与 `services/compact/compact.ts` 仍显式消费该 attachment family；相比重复 result:error / assistant finalize，`task_status` 更贴近仍未覆盖的 transcript/state surface。
- 最小验收口径：先在 server/open 闭环中补 1 条最小 official-compatible `attachment{type:"task_status",...}`，再在 `open --print` 输出新的 `task_status_attachment_*` 校验位，并复用既有 `go test / go build / fresh HTTP+Unix runtime` 验证链路。
