...
- 理由：`attachment:task_status` / `attachment:task_reminder` / `attachment:todo_reminder` 已补齐，下一批真实缺口会转到其它仍未覆盖的 attachment/message family，而不是继续重复 task-family / todo-family 的最小 reminder shape。
