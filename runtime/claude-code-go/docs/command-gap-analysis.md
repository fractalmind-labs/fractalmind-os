...
### 下一步要补的高频子路径：direct-connect 的最小 `hook_progress / hook_started` path
...
2. hook_response 收口后，最直接的下一个真实缺口变成 hook 生命周期仍不完整：当前 direct-connect 还缺最小 `hook_progress / hook_started` 事件与对应校验
