...
- 理由：`attachment:task_status` / `attachment:task_reminder` / `attachment:todo_reminder` / `attachment:compaction_reminder` / `attachment:auto_mode_exit` / `attachment:plan_mode_exit` / `attachment:plan_mode_reentry` / `attachment:date_change` / `attachment:verify_plan_reminder` 已补齐，下一批真实缺口会转到其它仍未覆盖的 attachment/message family，而不是继续重复 task-family / todo-family / compaction-reminder / auto-mode-exit / plan-mode-exit / plan-mode-reentry / date-change / verify-plan-reminder 的最小 shape。
