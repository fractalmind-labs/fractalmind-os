...
### 仍缺的关键用户向命令
- `server/open` 已具备最小 websocket ready/control/message 闭环，且 server 已补单实例 lockfile + session index + reconnect + detached-state + backend process lifecycle + 最小 tool execution / permission bridge；当前 `GET /sessions/{sessionId}` 可直接读到 `starting/running/detached/stopped` 与 `backend_status/backend_pid`，`resume` 期间可复用同一 live backend pid。最新一轮又在现有 success path 上补了最小 official-compatible assistant finalize lifecycle：server 额外发出 `message_start{message.usage}`、`message_delta{delta.stop_reason,usage}` 与 `message_stop`，assistant 最终消息同步保留 `tool_use` block 并回写 `stop_reason/usage`，open 摘要则显式落 `assistant_message_start_validated=true / assistant_message_delta_validated=true / assistant_message_stop_validated=true / assistant_stop_reason_validated=true / assistant_usage_validated=true`；该 path 仅保证最小 shape 对齐，不实现真实 token accounting、planner 或 richer UI 语义。当前又额外补了最小 `attachment:structured_output`：server 在 success path 发出 top-level `attachment{type:"structured_output",data:{text:<same result>}}`，`open --print` 新增 `structured_output_attachment_validated=true / structured_output_attachment_event=attachment:structured_output`，并与既有 `result.structured_output` 同时校验。本轮继续补了最小 `attachment:queued_command`、`attachment:task_status`、`attachment:task_reminder`、`attachment:todo_reminder`、`attachment:compaction_reminder`、`attachment:context_efficiency`、`attachment:auto_mode_exit`、`attachment:plan_mode`、`attachment:plan_mode_exit`、`attachment:plan_mode_reentry`、`attachment:date_change`、`attachment:ultrathink_effort`、`attachment:deferred_tools_delta`、`attachment:agent_listing_delta`、`attachment:mcp_instructions_delta`、`attachment:companion_intro`、`attachment:token_usage`、`attachment:output_token_usage`、`attachment:verify_plan_reminder` 与 `attachment:current_session_memory`：server 在 `system(task_notification)` 后依次发出 `attachment{type:"queued_command",prompt,commandMode:"task-notification"}`、`attachment{type:"task_status",taskId,taskType,status,description,deltaSummary,outputFilePath}`、`attachment{type:"task_reminder",content:[{id,status,subject}],itemCount}`、`attachment{type:"todo_reminder",content:[{content,status,activeForm}],itemCount}`，并在 `system(post_turn_summary)` 后依次发出 `attachment{type:"compaction_reminder"}`、`attachment{type:"context_efficiency"}`、`attachment{type:"auto_mode_exit"}`、`attachment{type:"plan_mode",reminderType:"full",planFilePath,planExists:false,isSubAgent:false}`、`attachment{type:"plan_mode_exit",planFilePath,planExists}`、`attachment{type:"plan_mode_reentry",planFilePath}`、`attachment{type:"date_change",newDate}`、`attachment{type:"ultrathink_effort",level:"high"}`、`attachment{type:"deferred_tools_delta",addedNames,addedLines,removedNames}`、`attachment{type:"agent_listing_delta",addedTypes,addedLines,removedTypes,isInitial,showConcurrencyNote}`、`attachment{type:"mcp_instructions_delta",addedNames,addedBlocks,removedNames}`、`attachment{type:"companion_intro",name,species}`、`attachment{type:"token_usage",used,total,remaining}`、`attachment{type:"output_token_usage",turn,session,budget}`、`attachment{type:"verify_plan_reminder"}` 与 `attachment{type:"current_session_memory",content,path,tokenCount}`；`open --print` 新增 `queued_command_validated=true / queued_command_event=attachment:queued_command`、`task_status_attachment_validated=true / task_status_attachment_event=attachment:task_status`、`task_reminder_attachment_validated=true / task_reminder_attachment_event=attachment:task_reminder`、`todo_reminder_attachment_validated=true / todo_reminder_attachment_event=attachment:todo_reminder`、`compaction_reminder_validated=true / compaction_reminder_event=attachment:compaction_reminder`、`context_efficiency_validated=true / context_efficiency_event=attachment:context_efficiency`、`auto_mode_exit_validated=true / auto_mode_exit_event=attachment:auto_mode_exit`、`plan_mode_validated=true / plan_mode_event=attachment:plan_mode`、`plan_mode_exit_validated=true / plan_mode_exit_event=attachment:plan_mode_exit`、`plan_mode_reentry_validated=true / plan_mode_reentry_event=attachment:plan_mode_reentry`、`date_change_validated=true / date_change_event=attachment:date_change`、`ultrathink_effort_validated=true / ultrathink_effort_event=attachment:ultrathink_effort`、`deferred_tools_delta_validated=true / deferred_tools_delta_event=attachment:deferred_tools_delta`、`agent_listing_delta_validated=true / agent_listing_delta_event=attachment:agent_listing_delta`、`mcp_instructions_delta_validated=true / mcp_instructions_delta_event=attachment:mcp_instructions_delta`、`companion_intro_validated=true / companion_intro_event=attachment:companion_intro`、`token_usage_validated=true / token_usage_event=attachment:token_usage`、`output_token_usage_validated=true / output_token_usage_event=attachment:output_token_usage`、`verify_plan_reminder_validated=true / verify_plan_reminder_event=attachment:verify_plan_reminder` 与 `current_session_memory_validated=true / current_session_memory_event=attachment:current_session_memory`，并要求它们在同 turn 命中；当前只保证固定 `commandMode="task-notification"`、稳定 task-notification XML prompt、`taskType=local_bash`、`status=completed`、稳定 `activeForm`、稳定 `planFilePath=.claude/plan.md`、`reminderType="full"`、`planExists=false`、`isSubAgent=false`、`newDate=2026-04-09`、`level="high"`、固定 `addedNames/addedLines/removedNames` 示例值，以及固定 `addedTypes=["explorer"]`、稳定单行 `addedLines`、`removedTypes=[]`、`isInitial=true`、`showConcurrencyNote=true`，还有固定 `addedNames=["chrome"]`、`addedBlocks=["## chrome\\nUse ToolSearch before browser actions."]`、`removedNames=[]`、固定 `name="Mochi"`、`species="otter"`、固定 `used=1024`、`total=200000`、`remaining=198976`、固定 `turn=256`、`session=512`、`budget=1024` 与固定 `content="Remember: keep this session focused."`、`path="MEMORY.md"`、`tokenCount=7` 的最小 envelope，不实现真实 queued-command 队列管理、去重/批处理、richer task lifecycle、todo 管理、reminder 策略、auto-compact 阈值、auto-mode classifier、真实 plan mode 生命周期、真实 plan reminder 调度、真实 plan re-entry 状态机、sub-agent plan orchestration、跨午夜 transcript flush、真实 snip pacing、token growth heuristic、richer reminder text、真实关键词触发、思考预算切换、真实 tools diff、scan context、dynamic discovery、真实 agent pool、动态过滤、订阅态判断、真实 MCP 连接池、动态 instructions 合并、断连回收、model/tool gate、真实 companion 开关、历史去重、气泡文案、user-addressed 行为、真实 token accounting、model-specific context window、env gate、动态 remaining 计算、真实 turn/session token accounting、feature gate、`budget=null` 分支、动态预算计算、真实 memory 文件读取、动态 tokenCount 统计、个性化记忆注入或 verify-plan 触发策略。此前的 `session_state_changed:requires_action`、`streamlined_text`、`streamlined_tool_use_summary`、`result:error_max_turns`、`result:error_max_budget_usd`、`result:error_max_structured_output_retries`、`update_environment_variables`、`control_request:mcp_oauth_callback_url`、`control_request:mcp_authenticate`、`control_request:channel_enable`、`control_request:hook_callback`、`control_request:elicitation`、`system:elicitation_complete`、`system:local_command_output`、`system:api_retry`、`system:files_persisted`、`system:task_started`、`system:task_progress`、`system:task_notification`、`system:session_state_changed`（running -> requires_action -> idle）、`system:post_turn_summary`、`system:compact_boundary`、`system:hook_started`、`system:hook_progress`、`system:hook_response`、multi-session / `max_sessions` guard + 单 session stop/cleanup 仍保持生效。
