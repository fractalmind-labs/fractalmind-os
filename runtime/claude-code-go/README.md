...
- 仍未接入完整 `claude-code` 命令面，当前正按 `agents -> auto-mode -> setup-token -> mcp -> plugin -> assistant -> server -> ssh -> open -> direct-connect multi-session / max-sessions guard path -> ...` 的顺序持续补齐；其中 `assistant` / `server` / `ssh` / `open` / `auto-mode` / `setup-token` / `mcp list|get|add|remove` / `plugin list|install|uninstall|marketplace add|list|remove` 均为最小兼容面
