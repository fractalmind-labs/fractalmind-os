<div align="center">

# FractalMind AI

**Heartbeat-driven operating system for AI agent teams.**

*Open-source infrastructure for governed autonomy, structured memory, multi-channel execution, and optional on-chain trust surfaces.*

[![Docs](https://img.shields.io/badge/docs-fractalmind--ai.github.io-4DA2FF)](https://fractalmind-ai.github.io)
[![GitHub Org](https://img.shields.io/badge/repos-18-181717?logo=github)](https://github.com/fractalmind-ai)
[![MIT License](https://img.shields.io/badge/license-MIT-green.svg)](https://github.com/fractalmind-ai)

</div>

---

## What FractalMind Is

FractalMind is no longer just a protocol story.

The current system is an **OS-first stack** for running AI agent teams through a repeatable control loop:

```
signal -> memory -> candidate OKR -> governance -> execution -> outcome -> evolution
```

That loop currently runs through:

- **`oh-my-code`** — the reference workspace and heartbeat-driven OS core
- **`agent-manager`** — tmux-based execution plane for agents
- **`fractalbot`** — multi-channel routing between humans and agents
- **`fractalmind-okrs`** — candidate OKR publication surface
- **`fractalmind-protocol`** — optional SUI trust layer when on-chain guarantees matter

## Mission / Vision / Values

### Mission
Turn requests, signals, and opportunities into execution that expands shared intelligence, durable progress, and long-term flourishing.

### Vision
Build a continuously improving operating system where humans and AI co-create the future together — deepening understanding of humanity, life, nature, and the wider universe while growing freedom, wisdom, and abundance.

### Values
- **Evidence before assertion**
- **Human–AI co-creation over zero-sum control**
- **Long-term flourishing over short-lived hype**
- **Recursive self-improvement in service of wisdom, freedom, and abundance**
- **Alignment with life, nature, and the evolving universe**
- **Transparency, reversibility, and operational honesty**

## Repository Surfaces

FractalMind AI currently spans **18 public repositories** across four surfaces:

1. **OS kernel & governance** — `oh-my-code`, `fractalmind-okrs`, `.github`, `agent-manager-skill`, `team-manager-skill`, `okr-manager-skill`
2. **Execution & collaboration** — `fractalbot`, `team-chat-skill`, `use-fractalbot-skill`, `agent-browser-skill`, `use-phone-skill`, `turbo-frequency-skill`
3. **Protocol & runtime** — `fractalmind-protocol`, `fractalmind-envd`, `explorer`, `openclaw-gateway-app`
4. **Applications & distribution** — `fractalmind-ai.github.io`, `typemind-android`

## Current Direction

The public shift is simple:

- **Old framing**: protocol-first, centered on SUI and on-chain organizations
- **Current framing**: OS-first, centered on heartbeat, structured memory, governed execution, and outcomes

The protocol still matters. It is just no longer the only story.

## Start Here

- **Docs**: https://fractalmind-ai.github.io
- **GitHub Org**: https://github.com/fractalmind-ai
- **Candidate OKRs**: https://github.com/fractalmind-ai/fractalmind-okrs
- **Explorer**: https://fractalmind-ai.github.io/explorer
- **Protocol (SUI Testnet)**: https://github.com/fractalmind-ai/fractalmind-protocol
